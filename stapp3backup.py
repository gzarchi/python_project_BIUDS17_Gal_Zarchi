...
if fetch_button:
    ...
    if selection != 'Other':
        st.write(f'fetch_data({selection_settings[0]}, {selection_settings[1]}, {selected_temp})  -->  df')
    ...
    # Process get_typed_selection_settings() for a new typed_input
        ...
        st.write('typed_selection_settings = get_typed_selection_settings({typed_selection}) -->  [name, tz] or None')

        # Simulate get_typed_selection_settings output
        typed_selection_settings = ['name', 'tz']  # None or ['name', 'tz']
...
if selected and not selected_default and not changed_default_temp:
    ...
    if set_as_default_button:
        st.write(f'set_as_default({selection_settings[0]}, {selection_settings[1]}, {selected_temp})  -->  replaces saved settings with initial_settings[0] in initial_settings')
...
if changed_default_temp or found_typed_selection_settings:
    ...
    # Process save_to_favorites_button
        ...
        st.write(f'save_settings({selection_settings[0]}, {selection_settings[1]}, {selected_temp})  -->  check if current selection_settings exists in initial_settings and saves to favorites')
